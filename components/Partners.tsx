...
// Official partner programs and developer memberships. AWS has no icon in the
// baked set (simple-icons dropped it), so its card carries the text wordmark.
const partners: { name: string; designation: string; slug?: string; color?: string }[] = [
  { name: "AWS", designation: "Official Partner" },
  { name: "Google Cloud", designation: "Official Partner", slug: "googlecloud" },
            ...
            <div className="flex h-full flex-col items-center justify-center rounded-[14px] border border-slate-200 bg-white px-4 py-6 text-center shadow-[0_1px_3px_rgba(14,27,46,0.04)] transition-all duration-[250ms] hover:-translate-y-[3px] hover:border-brand-200 hover:shadow-[0_14px_34px_-16px_rgba(14,102,194,0.2)]">
              <span className="flex h-10 items-center justify-center">
                {p.slug ? (
                  ...
                  <span className="font-display text-[26px] font-extrabold lowercase leading-none tracking-tight text-[#232F3E]">
                    aws
                  </span>
                )}
              </span>
