...
export function wrapEmail(
  ...
  <body style="margin:0;background:#f4f8fd;padding:24px">
    ...
    <div style="max-width:600px;margin:0 auto;background:#ffffff;border-radius:16px;padding:32px;font-family:Arial,Helvetica,sans-serif;color:#0E1B2E">
      <div style="font-size:20px;font-weight:800;color:#0E66C2;margin-bottom:24px">MnT Future</div>
...
export async function sendConfirmation(
  ...
  const html = wrapEmail(
    `<h1 style="font-size:20px;margin:0 0 12px">Confirm your subscription</h1>
     <p style="margin:0 0 20px">Tap the button below to confirm you'd like updates from MnT Future on AI-native, agent-ready commerce. If this wasn't you, just ignore this email.</p>
    ...
    { preheader: "Confirm your MnT Future newsletter subscription" }
  );
  try {
    const { error } = await resend.emails.send({
      from: FROM,
      to,
      replyTo: REPLY_TO,
      subject: "Confirm your MnT Future subscription",
      html,
    });
    if (error) return { sent: false, error: error.message };
    return { sent: true };
  ...
  for (let i = 0; i < recipients.length; i += chunkSize) {
    ...
    const messages = chunk.map((r) => {
      ...
      return {
        ...
        to: r.email,
        replyTo: REPLY_TO,
        subject: "Would you like updates from MnT Future?",
        html: wrapEmail(inner, {
          preheader: "Confirm to join the MnT Future newsletter",
          unsubscribeUrl: unsubscribeUrl(r.token),
        }),
        headers: {
          "List-Unsubscribe": `<${unsubscribeUrl(r.token)}>`,
