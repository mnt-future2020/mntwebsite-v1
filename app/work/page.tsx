      ...
      <section className="border-b border-bp-line bg-gradient-to-b from-[#FBFCFE] via-white to-white">
        <div className="mx-auto max-w-[1200px] px-5 pb-[72px] pt-[72px] sm:px-7 lg:pt-[88px]">
          <div className="animate-fade-up">
            ...
            <h1 className="mt-[18px] font-display text-[34px] font-extrabold leading-[1.12] tracking-[-0.02em] text-bp-ink sm:text-[46px]">
              Real platforms, shipped.
            </h1>
            <p className="mt-5 max-w-[620px] text-[17px] leading-[1.65] text-bp-mute">
      ...
      {GROUPS.map((g) => {
        ...
        return (
          <section key={g.key} className="mx-auto max-w-[1200px] px-5 pt-[72px] sm:px-7">
            <Reveal>
              <h2 className="font-display text-[26px] font-bold tracking-[-0.01em] text-bp-ink sm:text-[30px]">
                {g.title}
              </h2>
              <p className="mt-2.5 max-w-[620px] text-[15.5px] leading-[1.65] text-bp-mute">
                {g.desc}
              </p>
            </Reveal>
            ...
            <div className="mt-9 grid gap-[22px] md:grid-cols-2">
              {items.map((c, i) => (
                <Reveal key={c.slug} delay={i * 80}>
                  <Link
                    href={`/work/${c.slug}`}
                    className="group flex h-full flex-col overflow-hidden border border-bp-edge bg-white shadow-[0_1px_3px_rgba(14,27,46,0.05)] transition-all duration-[250ms] hover:-translate-y-1 hover:shadow-[0_20px_44px_-18px_rgba(14,102,194,0.22)]"
                  >
                    ...
                    <div className="flex flex-1 flex-col p-[26px]">
                      ...
                      <h3 className="mt-2.5 font-display text-[23px] font-bold text-bp-ink">{c.title}</h3>
                      <p className="mt-[9px] text-[14.5px] leading-[1.6] text-bp-mute">{c.tagline}</p>
              ...
              {g.key === ctaGroup && (
                <Reveal delay={items.length * 80}>
                  <div className="flex h-full flex-col justify-center border border-dashed border-[#D8E1EC] bg-white p-9 text-center">
                    <h3 className="font-display text-[21px] font-bold text-bp-ink">Your platform here next?</h3>
                    <p className="mx-auto mt-3 max-w-[360px] text-[14.5px] leading-[1.6] text-bp-mute">
                    ...
                    <div className="mt-[22px]">
                      <Link
                        ...
                        className="inline-flex items-center gap-2 bg-brand-700 px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-brand-800"
                      >
